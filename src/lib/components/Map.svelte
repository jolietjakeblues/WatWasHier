<script lang="ts">
  ...
  import { archaeologyPopup, bagPopup, defenceLinePopup, disappearedVillagePopup, minuutplanPopup, monumentNumber, municipalityHistoryPopup, perceelPopup, rcePopup, toponymPopup } from '$lib/feature-popup';
  ...
  function toponymsCollection(landscape: LandscapeContext | null) {
    const items = landscape?.toponyms.items ?? [];
    return {
      type: 'FeatureCollection' as const,
      features: items.map((toponym) => ({
        type: 'Feature' as const,
        geometry: { type: 'Point' as const, coordinates: [toponym.lon, toponym.lat] },
        properties: { label: toponym.label, kloekeCode: toponym.kloekeCode }
      }))
    };
  }
  function percelenCollection(landscape: LandscapeContext | null) {
    const items = landscape?.percelen.items ?? [];
    return {
      type: 'FeatureCollection' as const,
      features: items.map((perceel) => ({
        type: 'Feature' as const,
        ...
        properties: {
          ...
          areaSquareMeters: perceel.areaSquareMeters
        }
      }))
    };
  }
  ...
  $effect(() => { showBuildings; showHeritage; showFaces; showWorldHeritage; showArchaeology; showMunicipalityHistory; showMinuutplans; showToponyms; showPercelen; showDisappearedVillages; showDefenceLines; showHistorical; background; syncLayerVisibility(); updateMapUrl(); });
  $effect(() => {
  ...
  onMount(() => {
    ...
    void (async () => {
      ...
      map.on('load', () => {
        ...
        map.addSource('toponyms', { type: 'geojson', data: toponymsCollection(context) });
        map.addLayer({ id: 'toponyms-points', type: 'circle', source: 'toponyms', paint: { 'circle-radius': 5, 'circle-color': '#c026d3', 'circle-stroke-color': '#fff', 'circle-stroke-width': 1.5 } });
      ...
      map.on('click', (event) => {
        ...
        if (showToponyms && toponymHits[0]) {
          heritagePanelHtml = null;
          popup?.remove();
          popup = new maplibregl.Popup({ closeButton: true, maxWidth: '300px', offset: 10 })
            .setLngLat(event.lngLat)
            .setHTML(toponymPopup(toponymHits[0].properties))
            .addTo(map);
          return;
        }
        ...
        if (showPercelen && perceelHits[0]) {
          heritagePanelHtml = null;
          popup?.remove();
          popup = new maplibregl.Popup({ closeButton: true, maxWidth: '300px', offset: 10 })
            .setLngLat(event.lngLat)
            .setHTML(perceelPopup(perceelHits[0].properties))
            .addTo(map);
          return;
        }
